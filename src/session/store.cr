...
module Frost
  class Session
    ...
    abstract class Store
      # Searches a session by it's identifier. Returns `nil` if the session
      # doesn't exist or has expired. Returns a `Frost::Session` otherwise.
      ...
      # Extends the session's lifetime by updating it's expiry time. This will
      # be called instead of `#write_session` when the session has been loaded
      # but didn't changed.
      abstract def extend_session(session : Session) : Nil
