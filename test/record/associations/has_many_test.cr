...
module Trail
  class Record
    class Associations::HasManyTest < Minitest::Test
      def test_collection
        assert_equal [Comment.find("2de27d78-142a-4296-b76e-df1d1e1a8b35")],
          Post.find(1002).comments.to_a
        assert_equal ["one", "two", "five"], Post.find(1001).comments.pluck(:body)
      end

      def test_collection_memoization
        post = Post.find(1001)
        assert_same post.comments, post.comments
        refute_same post.comments, Post.find(1001).comments
      end
